pipeline {
    ...
    stages {
        stage('Run All') {
            steps {
                ...
                # 2. Запускаем QEMU в romulus папке (исправляем порт)
                cd romulus
                echo "Starting QEMU..."
                qemu-system-arm -m 256 -M romulus-bmc -nographic \
                  -drive file=obmc-phosphor-image-romulus-20250902012112.static.mtd,format=raw,if=mtd \
                  -net nic \
                  -net user,hostfwd=tcp:0.0.0.0:2222-:22,hostfwd=tcp:0.0.0.0:2443-:443 &
                QPID=$!
    ...
    post {
        always {
            // Сохраняем
            archiveArtifacts artifacts: 'my_results.txt, romulus/qemu_info.txt', allowEmptyArchive: true
        }
    }
}